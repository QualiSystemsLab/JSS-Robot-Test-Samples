...
class QualiAPISession():
	def __init__(self, host, username='', password='', domain='Global', timezone='UTC', datetimeformat='MM/dd/yyyy HH:mm', token_id='', port=9000):
		...
		if sys.version_info.major >= 3:
			self._auth_code = "Basic {0}".format(login_result.content.decode("ascii")[1:-1])
		else:
	...
	def GetReservationAttachment(self, sandbox_id, filename, save_path):
		...
		"""
		get_result = requests.post(self._api_base_url + "/Package/GetReservationAttachment", json={"ReservationId":sandbox_id, "FileName": filename}, headers={"Authorization": self._auth_code})
	...
	def DeleteFileFromReservation(self, sandbox_id, filename):
		...
		"""
		delete_result = requests.post(self._api_base_url + "/Package/DeleteFileFromReservation", json={"reservationId": sandbox_id, "FileName": filename}, headers={"Authorization": self._auth_code})
